package InterfaceFramework.phpwind;
...
public class RequestUtils {
    ...
    //传递Map类型的参数组装成url返回
    private static String getUrl(String url, Map<String, String>... parameter) {
        // List<NameValuePair> param = new ArrayList<NameValuePair>();
        logger.info("组装url");
        //把参数转换为一个迭代器
        Iterator<String> itr = parameter[0].keySet().iterator();
...
//            }
    ...
    private static String toString(CloseableHttpResponse response){
            ...
            if (response != null) {
                ...
                if (statuCode != 200) {
                    if(response.getEntity().getContentLength()==0){
                        logger.error("返回实体内容为空");
                        return null;
                    }
                    logger.error("访问失败，返回的状态码为：" + statuCode + response.getStatusLine());
                    response.close();
                } else {
                    // 将响应内容转化成string
        ...
        //打印jsonObject内容
        logger.info("jsonObject"+stringresponse);
        return stringresponse;
    }
